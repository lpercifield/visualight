...
module.exports = function(app, exp, MongoStore) {
	...
	app.configure(function(){
		...
		//app.use(exp.session({ secret: 'super-duper-secret-secret', store: new MongoStore({url:'mongodb://nodejitsu:ab124187f3e14972d1502824506e7123@linus.mongohq.com:10052/nodejitsudb6620458662'},function(){console.log('MongoStore')}) })); // setup mongostore to use the db url and credentials
		app.use(exp.session({ secret: 'super-duper-secret-secret', store: new MongoStore({db:'visualightdev'},function(){console.log('MongoStore')}) }));
